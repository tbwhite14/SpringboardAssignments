function App ()
{
	...
	const phenomenaStatuses = spacePhenomena.map((p) => {
		let rand = Math.floor(Math.random()*3);
		p={...p,status: observationStatuses[rand]};
	...
	return (
		<div>
			<ul>
				{phenomenaStatuses.map((t) => {
					let line = <li key={t.id}>{t.name} {t.emoji} {t.status} <b>{t.status === "🚀 Prime for Study" ? "Grab your best gear!" : ""}</b></li>;
					return line;
				})}
			</ul>
		</div>
